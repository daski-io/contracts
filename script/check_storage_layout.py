#!/usr/bin/env python3
"""Compare normalized upgradeable storage layouts with the reviewed baseline."""
...
FIRST_DERIVED_STORAGE = {
    ...
    "ValidationRegistry": ("100", "identityRegistry"),
    "ReputationStorage": ("100", "_records"),
}


class LayoutError(RuntimeError):
    """Raised when an upgradeable storage layout is unsafe or unexpected."""
...
def inspect(repo: Path, forge: str) -> dict[str, Any]:
    ...
    for contract in CONTRACTS:
        ...
        require(
            result.returncode == 0,
            result.stderr.strip() or f"forge inspect failed: {contract}",
        )
