...
abstract contract CanonicalSafeFixture is Test {
    ...
    function _executeSafe(address safe, address target, bytes memory data) internal {
        ...
        (uint8 v1, bytes32 r1, bytes32 s1) = vm.sign(first, digest);
        (uint8 v2, bytes32 r2, bytes32 s2) = vm.sign(second, digest);
        bool success = ICanonicalSafe(safe)
            .execTransaction(
                target, 0, data, 0, 0, 0, 0, address(0), payable(address(0)), abi.encodePacked(r1, s1, v1, r2, s2, v2)
            );
        assertTrue(success);
    }
}
